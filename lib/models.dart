...
class Player {
  ...
  Table table = Table(number: -1, name: "Bye");
...
class Table {
  ...
  void addPlayer(Player player, int num) {
    if (num == 1) {
      player1 = player;
    }
    if (num == 2) {
...
class Division {
  ...
  Division copy() {
    Division ret = Division(name, number);
    ret.players = [...players];
    return ret;
  }
}

class CombinedBracket{

}

class Bracket {
  List<Division> divisions = [];
  List<Division> mergedDivisions = [];
  List<Table> tables = [];
  String name = "";
  int rounds = 0;
  ...
  void matchTables(int i) {
    while (i >= tables.length) {
      tables.add(Table(number: tables.length + 1, bestOf: bestOf));
    }
    tables = tables.sublist(0, i);
  }

  Bracket matchDivision(int i) {
    while (i >= divisions.length) {
      divisions
          .add(Division("Division ${divisions.length + 1}", divisions.length));
    }
  ...
  void update() {
    ...
    for (Division division in divisions) {
      if (division.players.length < 6 &&
          division.players.isNotEmpty &&
          mergeDivisions &&
          mergedDivisions.isNotEmpty) {
        mergedDivisions.last.name += " + ${division.name}";
        mergedDivisions.last.players.addAll(division.players);
      } else {
        mergedDivisions.add(division.copy());
        mergedDivisions.last.number = mergedDivisions.length;
      }
    }
    print(mergedDivisions);
    int tableNum = 0;
    int playerCount = 0;
    for (Division division in mergedDivisions) {
      tableNum += division.players.length ~/ 2;
      playerCount++;
    }
    while(1<<rounds > playerCount){
      rounds++;
    }
    print(tableNum);
    matchTables(tableNum);
